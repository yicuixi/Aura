...
def clean_runtime_data():
    ...
    # 删除文件
    for file_pattern in runtime_files:
        if "*" in file_pattern:
            # 处理通配符
            import glob
...
def clean_docs_and_templates():
    """清理文档和模板文件（如果不需要WebUI）"""
    print("📄 清理WebUI相关文件...")
    
    # 询问是否保留WebUI相关文件
    response = input("  ❓ 是否保留WebUI相关的docs和templates目录? (y/N): ")
...
def fix_aura_fixed():
    ...
    # 替换硬编码的个人信息
    original_text = "Lydia是光学研二硕士生，研究OAM相位重建+少样本识别，正在准备英伟达面试。"
    replacement_text = "用户是一名学生/研究人员，请根据实际情况自定义此部分。"
...
def main():
    ...
    # 检查是否在正确的目录
    if not os.path.exists("aura_fixed.py") and not os.path.exists("memory.py"):
        print("❌ 当前目录似乎不是Aura项目目录")
        print("请在Aura项目根目录下运行此脚本")
        return
    
    # 询问确认
    confirm = input("⚠️ 此操作将删除运行时数据和冗余文件，确定继续吗? (y/N): ")
    if confirm.lower() != 'y':
        print("❌ 用户取消操作")
        return
    ...
    try:
        ...
        print("  3. 更新 data/example_profile.md 为您的信息")
        print("  4. 运行 git add . && git commit -m 'Clean project and remove personal info'")
        
        print("\n💡 提示:")
        print("  - 个人信息备份在 personal_backup/ 目录中")
        print("  - 如需恢复，可以从备份目录复制回来")
        print("  - 清理后的项目可以安全地提交到Git")
